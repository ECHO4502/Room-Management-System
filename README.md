...
面向民宿私有部署的客房管理系统：默认无需登录即可开箱即用，支持全日租、钟点房、长租三种计费方式与多门店；可选启用账号密码登录（公网 / 内网穿透部署建议启用）。
...
- 数据与权限：默认无需登录即可使用，适合单机 / 局域网私有部署；可选启用账号密码登录（见下）；房间删除为软删除（停用），订单删除为物理删除，不可恢复。
- 登录鉴权（默认关闭，可启用）：系统内置账号+密码登录（无注册、客户端无需保存 token，HttpOnly Cookie 会话，会话不自动过期，登出或修改密码后失效）；将系统设置 `settings.auth_enabled` 置为 1 即启用，启用后除 `/api/auth/*` 外所有 API 均需登录；若尚未设置账号，页面会先要求设置管理员账号与密码（密码需 8 位以上且同时包含字母和数字，连续输错会触发临时限速）；`/api/auth/status`、`/api/auth/setup`、`/api/auth/login`、`/api/auth/logout`、`/api/auth/password` 分别提供状态查询、初始设置、登录、登出与修改密码能力；经内网穿透 / 反向代理的公网访问（X-Forwarded-For / X-Real-IP 非私有网段）时自动隐藏备份、二维码与数据库路径（仅本机 / 局域网可见）。
...
| GET/POST/PUT | `/api/auth/*` | 登录鉴权接口（状态 / 初始设置 / 登录 / 登出 / 修改密码；默认关闭，可启用） |
...
| GET | `/api/statistics/today` | 指定日期统计（应到 / 应退 / 收入、销售额与回款） |
