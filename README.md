...
- 计费方式：`full_day` 全日租（默认入住日 14:00 入住、离店日 12:00 退房，按晚计费）、`hourly` 钟点房（按日租原价计费：至少收一晚日租价，订单时间每超过一个次日 12:00 即多收一晚，上限随超时逐日上调、`long` 长租（按日计价，支持日结收款或一次性结算）。
...
- 房间管理：启用 / 停用、维修开关、需打扫标记；状态筛选含空闲 / 已预订 / 已入住 / 维修 / 需打扫，另设启用状态筛选。
