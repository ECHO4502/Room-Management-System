...
- 订单自动维护：默认关闭；设置页提供启用开关（开启时风险提示并自动备份，不处理长租订单），启用后可分别开启自动入住 / 自动退房 / 自动续住（退房与续住互斥，后启用生效）；订单详情可对单笔订单开启到点自动退房或自动续住（长租订单除外，单订单优先于全局）；自动操作会写入订单与收支备注并记录日志（设置页可查看，含订单日期/房号/客人并可查看详情），支持全局 / 单笔回滚（回滚后记录移除并关闭对应自动化）。
