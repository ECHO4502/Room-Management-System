...
- 订单自动维护：默认关闭；设置页提供启用开关（开启时风险提示并自动备份，不处理长租订单），启用后可分别开启自动入住 / 自动退房 / 自动续住（退房与续住互斥，后启用生效）；订单详情可对单笔订单开启到点自动入住 / 自动退房 / 自动续住（长租订单除外，单订单优先于全局）；自动续住按日单价累加金额（钟点房不超过日租价）且备注累计续住次数；自动维护处理过的订单在房态网格以绿色边框提示；自动操作会写入订单与收支备注并记录日志（设置页可查看，含订单日期/房号/客人并可查看详情），支持全局 / 单笔回滚（回滚后记录移除并关闭对应自动化）。
