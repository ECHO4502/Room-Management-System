...
- 登录鉴权（预留，默认关闭）：使用账号+密码登录（无注册、客户端无需保存 token，HttpOnly Cookie 会话，会话不自动过期（登出或修改密码后才失效））；首次启用（settings.auth_enabled=1 且未设置账号）时页面将先要求设置管理员账号与密码，后续登录进入；`/api/auth/*` 提供状态、初始设置、登录、登出与修改密码接口，当前版本默认不启用；经内网穿透/反向代理的公网访问（X-Forwarded-For 非私有网段）时自动隐藏备份、二维码与数据库路径（仅本机/局域网可见）。
...
| GET/POST/PUT | `/api/auth/*` | 账号密码登录（预留，默认关闭） |
