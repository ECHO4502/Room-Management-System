...
def create_room(conn: sqlite3.Connection, data: schemas.RoomCreate) -> dict:
    ...
    cur = conn.execute(
        ...
        " VALUES (?, ?, ?, ?, ?, ?, 1, ?, ?)",
        (number, data.room_name.strip(), data.room_category.value, data.base_price, hourly,
...
def _order_joined(conn: sqlite3.Connection, order_id: int) -> dict | None:
    row = conn.execute(
        "SELECT o.*, r.room_number, r.room_name, r.base_price, r.hourly_price FROM orders o"
...
def get_all_orders(conn: sqlite3.Connection, status: str | None = None,
    ...
    """
    sql = ("SELECT o.*, r.room_number, r.room_name, r.base_price, r.hourly_price,"
